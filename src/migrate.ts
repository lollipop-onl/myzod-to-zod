...
import {
	collectMyzodImportDeclarations,
 ...
 */
export function migrateMyzodToZodV3WithIssues(
 ...
 */
function handleSpecialTransformations(
	sourceFile: SourceFile,
	myzodName: string,
) {
	...
	const coerceTransformations: {
		callExpr: CallExpression;
		typeName: string;
	}[] = [];
	const literalsTransformations: {
		callExpr: CallExpression;
		args: string[];
	}[] = [];
					...
					if (Node.isCallExpression(baseExpression)) {
						...
						if (Node.isPropertyAccessExpression(baseCallExpression)) {
							...
							if (
								...
								rootExpression.getText() === myzodName
							) {
 ...
 */
function handleNamedImportTransformations(
	sourceFile: SourceFile,
	namedImports: string[],
) {
			...
			if (Node.isCallExpression(parent) && parent.getExpression() === node) {
				switch (name) {
					...
					case "literals": {
						...
						const literalUnions = literalValues
 ...
 */
function detectManualMigrationIssues(
	sourceFile: SourceFile,
				...
				if (Node.isPropertyAccessExpression(parent)) {
					...
					if (rootId === "myzod") {
						...
						issues.push({
							...
							description:
								"Update ValidationError handling to use result.success pattern",
							line: lineNumber,
							snippet: snippet,
						});
					}
		...
		if (Node.isQualifiedName(typeName)) {
			...
			if (
				...
				issues.push({
					type: "validation-error",
					description:
						"Update ValidationError type references for zod error handling",
					line: lineNumber,
					snippet: snippet,
				});
			}
 ...
 */
function detectValidationErrorImports(
	sourceFile: SourceFile,
	issues: ManualMigrationIssue[],
) {
